...
impl<'a> State<'a> {
    pub async fn new(window: &'a winit::window::Window) -> Self {
        ...
        let diffuse_texture = texture::Texture::from_bytes(&device, &queue, diffuse_bytes, Some("Happy Tree Texture")).expect("Error loading texture");
        ...
        Self{ surface, device, queue, config, size, window, render_pipeline, vertex_buffer, index_buffer, diffuse_texture, diffuse_bind_group }
    }
